...
export const getProductImageUrl = (imagePath = '', image = '') => {
  ...
  // Product APIs normally return a full uploads URL in `_image_path`. If that
  // environment variable is missing, the API sends `undefined/products`; use
  // the configured API host so a broken URL never falls back to the site logo.
  if (/^https?:\/\//i.test(cleanImagePath)) {
    return `${cleanImagePath}/${cleanImage}`
  }

  try {
    const apiOrigin = new URL(getAdminApiBaseUrl()).origin
    return `${apiOrigin}/uploads/products/${cleanImage}`
  } catch {
    return cleanImagePath ? `${cleanImagePath}/${cleanImage}` : `/${cleanImage}`
  }
}
