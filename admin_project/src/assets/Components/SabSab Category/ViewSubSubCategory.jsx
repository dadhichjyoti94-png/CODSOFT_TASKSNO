...
export default function ViewSubSubCategory() {
  ...
  useEffect(() => {
    axios.post(`${import.meta.env.VITE_API_BASE_URL}/sub-sub-category/view`, {   //API CALL
      name: filterData.name,             //BACKEND M DATA JA RAHA H NAME SE FILTER
      parent_category_id: filterData.parent_category_id,
      page: currentPage,
      sub_category_id: filterData.sub_category_id,                  //""  CURRENT PAGE
      limit: 0
    })
      .then((result) => {
        if (result.data._status == true) {           //BACKEND M CHECK KAREGA KI STATUS TRUE
          ...
          setSubSubCategory([]);                      //STATUS FALSE HOGA TO ESME JAYGA
          iziToast.error({
            title: 'error',
            message: 'something went wrong',
            position: 'topRight',
  ...
  }, [filterData.name, filterData.parent_category_id,filterData.sub_category_id, currentPage, apiStatus]);
  ...
  if (parentId) {
    ...
    axios.post(
      `${import.meta.env.VITE_API_BASE_URL}/sub-sub-category/sub-category`,
      {
        status: true,
        parent_category_id: parentId
      }
...
SetSubCategory([]);
  ...
  //All check box
   ...
   const selectAllCheckBox = () => {
    if (SubSubCategory.length == selectedRecord.length) {
      setSelectedRecord([]);
    } else {
      setSelectedRecord([]);
      var checkboxValues = [];
      SubSubCategory.forEach(element => {
        ...
        buttons: [
          [
            ...
            function (instance, toast) {
              axios.post(`${import.meta.env.VITE_API_BASE_URL}/sub-sub-category/delete`, {
                id: selectedRecord,
              })
                .then((result) => {
                  if (result.data._status == true) {
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="mb-5">
                ...
                <select
                  ...
                  onChange={(e) => setFilterValues((previous) => ({ ...previous, sub_category_id: e.target.value }))}
                  className="text-[17px] border border-slate-300 rounded-lg
                   focus:ring-2 focus:ring-indigo-400 focus:border-indigo-500
                         block w-full py-2.5 px-3"
                >
                ...
                <tbody>
                  {
                    SubSubCategory.length > 0
                      ...
                      SubSubCategory.map((v, i) => {
                        return (
                          <tr key={v._id} className="bg-white border-b">
                            ...
                            <td className="px-2 py-4">
                              {v.parent_category_id?.name || "N/A"}
                            </td>

                            <td className="px-2 py-4">
                              {v.sub_category_id?.name || "N/A"}
                            </td>
                      ...
                      <tr className="bg-white border-b">
                        <td colSpan={9} className="px-2 py-4 text-center font-bold">
                          No Record
                        </td>
                      </tr>
                  }
